create or replace procedure snpb_build_scadentar_corona
-- procedure    : snpb_build_scadentar_corona
-- Creat de     : teo, adrian teodorescu
-- Creat la     : 19-03-2003

-- Modificat de : Alexandra Marinescu
-- Modificat la : 11-04-2020
-- Scop         : scrie in tabela temporara cre_scadentar_corona_tmp pt a evidentia la final esalonarea dobanzii gratie corona
...
--  v_procent       number;                     -- del by teo @ 27-11-2003
    ...
    v_data_stop date; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_sold_ext number; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_nr_rate_ext number; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_count number; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_suma_lunara_esalonata number; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_data_prima_rata_dob date; -- add AM@08.04.2020 SRSNPB-700 Pandemie
    v_nr number; -- add AM@08.04.2020 SRSNPB-700 Pandemie
...
-- teo 12-05-2009
                ...
                ,t.clasificare_1_sv -- add AM@08.04.2020 SRSNPB-700 Pandemie
-- end 12-05-2009
...
-- / teo @ 18-12-2006
    ...
    v_data_stop   := to_date(snpb_get_oatribut_tr_null(r_credit.tranzactie_id, 'DATA_STOP_GR'),'DD-MM-RRRR');
    
    if v_flag_corona in ('MORAT','MORBA') then
       for rec in (
           select nvl(cp.suma_dobanda_gratie,0) suma_dobanda_gratie, nvl(cp.suma_lunara_esalonata,0) suma_lunara_esalonata
                , nvl(cp.suma_com_gratie,0) suma_com_gratie, nvl(cp.com_lunar_esalonat,0) com_lunar_esalonat      
           from   snpb_credite_extensii cp
           where  cp.credit_id = r_credit.credit_id) loop

           if rec.com_lunar_esalonat = 0 then
               rec.com_lunar_esalonat := null;
           end if;
           
           v_sold_ext := rec.suma_dobanda_gratie + rec.suma_com_gratie;
           
           if v_sold_ext <> 0 then

           -- daca e PF ipotecar atunci avem 60 de rate sau cate au mai ramas daca am iesit deja din gratie
           if r_credit.clasificare_1_sv in ('PF02','PF04','PF05','PF06','PF07','PF09','PF14') then
               v_nr_rate_ext := 60;
               if v_data_stop < v_data_sys then
                   -- diminuam nr de rate ramase
                   select count(*)
                     into v_count
                     from cre_rate r
                    where r.data >= v_data_stop
                      and r.credit_id = r_credit.credit_id;
                               
                   v_nr_rate_ext := v_nr_rate_ext - v_count;
               end if;
               
           -- daca e alt tip si suntem in perioada de gratie
           else
               if v_data_stop < v_data_sys then
                   v_nr_rate_ext := a_len;
               else
           ...
           v_suma_lunara_esalonata := rec.suma_lunara_esalonata;
           v_nr := 1;
               
           for cor in (select * 
                       from   cre_scadentar_tmp
                       where  is_dobanda = 'D'
                       order by data) loop
                   
                   if v_nr = v_nr_rate_ext then
                       v_suma_lunara_esalonata := v_sold_ext;
                   end if;
                   
                   v_sold_ext := v_sold_ext - v_suma_lunara_esalonata - nvl(rec.com_lunar_esalonat,0);
                          
                   insert into cre_scadentar_corona_tmp(
                        credit_id, data, rata, dobanda, sold, rata_platita, is_rata, is_dobanda, dobanda_precalculata, comisioane, comisioane_desc
                    ) values (
                       r_credit.credit_id, cor.data, null, v_suma_lunara_esalonata, v_sold_ext, null, 'X', 'X', null, rec.com_lunar_esalonat, 'ESALONARE CORONA'
                    );
                            
                   v_nr := v_nr + 1;
                  
                  -- iesim din loop daca am inserat nr de rate pt esalonare
                  exit when v_nr > v_nr_rate_ext;    
            end loop;
        end if;
        
        --modificam dobanda la prima scadenta numai cand suntem in perioada de gratie
        if v_data_stop >= v_data_sys then
            select min(data)
            into   v_data_prima_rata_dob
            from   cre_scadentar_tmp
            where  is_dobanda = 'D';
            
            -- !!! PDP_DATA daca sunt scadente inainte de DATA STOP CORONA, atunci stergem acele scadente
            if v_data_prima_rata_dob < v_data_stop then
                delete from cre_scadentar_tmp
                where is_dobanda = 'D'
                and data < v_data_stop;
            end if;
                        
            update cre_scadentar_tmp
            set    dobanda = dobanda - rec.suma_dobanda_gratie
            where  is_dobanda = 'D'
            and    data = v_data_prima_rata_dob;
         end if; 
